...
const LogInForm = () => {
  ...
  useEffect(() => {
    const users = async () => {
      ...
      const authorized = userList.users.find(
        (user) =>
          user.username === checkLog.username && user.psw === checkLog.psw,
      );
      if (authorized) {
  ...
  return (
    <div className="form-container">
      ...
      {wrongLog ? (
        <p>You spelled something wrong, please try again...</p>
      ) : (
        <p>
          Forgot your password?{" "}
          <a onClick={() => alert("LOL, just make a new one!")}>Click here</a>{" "}
          for support!
        </p>
      )}
      <p>
